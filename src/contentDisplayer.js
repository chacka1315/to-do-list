...
function ContentDisplayer() {
  ...
  const generateAddTaskBtn = (divProject, projectId) => {
    if (projectId === projectManager.projects[0].id) {
      // we don't want to add a add task button bellow for the general tasks
      return;
    } else {
    ...
    !projectManager.projects.length
      ...
      : projectManager.projects.forEach((project) => {
          ...
          if (!project.tasks.length) {
            ...
            const text = document.createElement("p");
            text.textContent =
              "No tasks here ! Click the button bellow to add your tasks...";
            text.style.fontStyle = "italic";
            text.style.color = "gray";
            ...
            project.tasks.forEach((task) => {
              task.detailsOpen
                ? generateTaskWithDetailsBlock(divProject, project, task)
  ...
  const displayOneProject = (projectId) => {
    content.textContent = "";
    const projectClicked = projectManager.projects.find(
    ...
    if (projectClicked) {
      ...
      if (!projectClicked.tasks.length) {
        ...
        projectClicked.tasks.forEach((task) => {
          task.detailsOpen
            ? generateTaskWithDetailsBlock(divProject, projectClicked, task)
            : generateSimpleTaskBlock(divProject, projectClicked, task);
        });
      }
